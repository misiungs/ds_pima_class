...
class Diagnostic():
    """
    Plot diagnostics for logistic regression.
    https://robert-alvarez.github.io/2018-06-04-diagnostic_plots/
    """
    def __init__(self, X, y, lr, C):
        self.df = pd.concat([X,y], axis=1)
        prob = lr.predict_proba(X)
        self.df['position'] = np.arange(len(self.df))
        self.df['log_odds'] = np.log(prob[:,1]/prob[:,0])
        self.model = Ridge(alpha=(1./(2*C))).fit(X, self.df['log_odds'])
        #statsmodels is used instead of sklearn because it has useful built-in methods
        #alpha have to be squared because sklearn minimizes l2 norm of error 
        #and statsmodel sum of squared residuals
        #model = sm.OLS(df_f['log_odds'], sm.add_constant(X))
        #rid = model.fit_regularized(L1_wt=0, alpha=((1./(2*lr.C)))**2)
        #this is equivalent to: rid = Ridge(alpha=(1./(2*lr.C))).fit(X, df_f['log_odds'])
        # Leverage is computed as the diagonal of the projection matrix of X
        # TODO: whiten X before computing leverage
        #code from yellowbrick
        # https://www.scikit-yb.org/en/latest/_modules/yellowbrick/regressor/influence.html#cooks_distance
        ...
        residuals = self.df['log_odds'] - fitted
    ...
    def plotResFit(self, annot=0, ax=None):
        #sns.lmplot(x='fitted', y='residual', data=self.df, hue = 'Outcome',
        ...
        sns.scatterplot(x='fitted', y='residual', hue='Outcome', data=self.df, ax=ax[0])
    ...
    def plotResSqrtFit(self, annot=0, ax=None):
        ...
        if not ax:
            ax = plt.subplot()
        sns.scatterplot(x='fitted', y='residual_sqrt', hue='Outcome', data=self.df, ax=ax)
    ...
    def plotResLev(self, annot=0, ax=None):
        if not ax:
            ax = plt.subplot()
        sns.scatterplot(x='leverage', y='residual_stud', hue='Outcome', data=self.df, ax=ax)
    ...
    def removeMax(self, type='residual', number=0, return_df = False):
        ...
        if return_df:
            df_ret = self.df.drop(['log_odds', 'residual', 'fitted', 'residual_stud', 'residual_sqrt',
    ...
    def returnData(self):
            df_ret = self.df.drop(['log_odds', 'residual', 'fitted', 'residual_stud', 'residual_sqrt',
                  'leverage', 'distance', 'p_values', 'position'], axis=1).reset_index(drop=True)
            X = df_ret.loc[:, df_ret.columns != 'Outcome']
            y = df_ret.loc[:, df_ret.columns == 'Outcome']
            return X, y
